...
def run_diagnostics(lp_do, lp_di, hp_do, hp_di, k_lp, k_hp, k_is):
    ...
    # natural frequencies at 10 RPM (approx 0)
    eigvals_0 = rotor.solve_eigenvalues(10.0 * 2.0 * np.pi / 60.0)
    freqs_0, _ = rotor.extract_natural_frequencies(eigvals_0, n_modes=6)
    print(f"Natural Frequencies at 0 RPM [Hz]: {[round(x, 1) for x in freqs_0]}")
    ...
    crits_rpm = [int(c["rpm"]) for c in criticals_1x]
    print(f"1x Critical Speeds [RPM]: {crits_rpm}")
    for c in criticals_1x:
        print(f"  Mode: {c['mode']} at {int(c['rpm'])} RPM (Freq: {c['freq_hz']:.1f} Hz)")

    rpm_idle = 21000.0
    rpm_max = 38500.0
    
    in_range = [r for r in crits_rpm if rpm_idle <= r <= rpm_max]
    below_idle = [r for r in crits_rpm if r < rpm_idle]
    above_max = [r for r in crits_rpm if r > rpm_max]
    
    print(f"Critical speeds in operating range: {in_range}")
    if below_idle:
        mb = (rpm_idle - max(below_idle)) / max(below_idle) * 100.0
        print(f"Margin below idle: {mb:.1f} % (req >10%) (Max below: {max(below_idle)} RPM)")
    else:
        print("No critical speeds below idle.")
    if above_max:
        ma = (min(above_max) - rpm_max) / rpm_max * 100.0
        print(f"Margin above max: {ma:.1f} % (req >15%) (Min above: {min(above_max)} RPM)")
    else:
        print("No critical speeds above max.")

# Let's run diagnostics for a very stiff setup:
# LP do = 34mm, lp_di = 22mm
# HP do = 50mm, hp_di = 36mm
# k_lp = 1.0e8, k_hp = 12.0e8, k_is = 4.5e8
print("DIAGNOSTICS:")
run_diagnostics(0.034, 0.022, 0.050, 0.036, 1.0e8, 12.0e8, 4.5e8)
