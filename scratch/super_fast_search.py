...
def main():
    # Only sweep 24 most promising configurations
    # LP do: 28, 30, 32, 34 mm
    # LP wall: 5.0 or 6.0 mm -> lp_di = lp_do - 2*thick
    # HP di: lp_do + 2mm (clearance)
    # HP wall: 6.0 or 7.0 mm -> hp_do = hp_di + 2*thick
    # k_lp = 1.0e8, k_hp = 12.0e8, k_is = 4.5e8 (standard stiff bearings)
    tasks = []
    for lp_do in [0.028, 0.030, 0.032, 0.034]:
        for lp_thick in [0.005, 0.006]:
            lp_di = lp_do - 2 * lp_thick
            hp_di = lp_do + 0.002
            for hp_thick in [0.006, 0.007]:
                hp_do = hp_di + 2 * hp_thick
                # 3 options of stiffness
                for (k_lp, k_hp, k_is) in [(1.0e8, 12.0e8, 4.5e8), (1.2e8, 14.0e8, 5.0e8)]:
