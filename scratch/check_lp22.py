...
# Check configurations with LP DO = 22.0 mm (Current CAD LP DO!)
# HP DO: 36.0 mm (Current CAD HP DO!)
# We sweep k_lp, k_hp, k_is to pull Mode 3 below 18.9k RPM.
# k_lp: 0.15e8 to 0.35e8 N/m
# k_hp: 8.0e8 to 12.0e8 N/m
# k_is: 3.0e8 to 5.0e8 N/m
lp_do = 0.022
lp_di = 0.014
hp_do = 0.036
hp_di = 0.024
...
test_cases = []
for k_lp in [0.15e8, 0.20e8, 0.25e8, 0.30e8, 0.35e8]:
    for k_hp in [8.0e8, 10.0e8, 12.0e8]:
        for k_is in [3.0e8, 4.0e8, 5.0e8]:
            test_cases.append((k_lp, k_hp, k_is))

print(f"Checking {len(test_cases)} soft configurations with LP DO = 22.0 mm, HP DO = 36.0 mm...", flush=True)
for idx, tc in enumerate(test_cases):
    k_lp, k_hp, k_is = tc
    safe, crits, mb, ma = run_eval(lp_do, lp_di, hp_do, hp_di, k_lp, k_hp, k_is)
    if safe:
        print(f"FOUND SAFE LP22 CONFIG: k_lp={k_lp/1e6:.1f} MN/m, k_hp={k_hp/1e6:.1f} MN/m, k_is={k_is/1e6:.1f} MN/m")
        print(f"  crits={crits}, margin_below={mb:.1f}%, margin_above={ma:.1f}%", flush=True)
print("Done soft check for LP22.", flush=True)
