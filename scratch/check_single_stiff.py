...
# Test list of stiffening configurations
# We want to push Mode 2 above 44.25k RPM.
# What if we increase LP shaft diameter to 45 mm, but keep DI = 37 mm (et kalinligi 4 mm) or DI = 35 mm (et kalinligi 5 mm)?
# Let's test outer diameters 42, 44, 46, 48 mm with et kalinligi 5 mm, and very stiff bearings.
test_cases = [
    # lp_do, lp_di, hp_do, hp_di, k_lp, k_hp, k_is
    (0.044, 0.034, 0.058, 0.046, 2.0e8, 16.0e8, 6.0e8),
    (0.044, 0.034, 0.058, 0.046, 2.5e8, 18.0e8, 7.0e8),
    (0.046, 0.036, 0.060, 0.048, 2.0e8, 16.0e8, 6.0e8),
    (0.046, 0.036, 0.060, 0.048, 2.5e8, 18.0e8, 7.0e8),
    (0.048, 0.038, 0.062, 0.050, 2.0e8, 16.0e8, 6.0e8),
    (0.048, 0.038, 0.062, 0.050, 2.5e8, 18.0e8, 7.0e8),
]

for idx, tc in enumerate(test_cases):
    safe, crits, mb, ma = run_eval(*tc)
    print(f"Test case {idx+1}: safe={safe}, crits={crits}, margin_below={mb:.1f}%, margin_above={ma:.1f}%", flush=True)
