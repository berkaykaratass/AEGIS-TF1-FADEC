...
def run_eval(lp_do, lp_di, hp_do, hp_di, k_lp, k_hp, k_is):
    ...
    crits = [int(c["rpm"]) for c in criticals_1x]
    in_range = [r for r in crits if rpm_idle <= r <= rpm_max]
    below_idle = [r for r in crits if r < rpm_idle]
    above_max = [r for r in crits if r > rpm_max]
    ...
    return safe, crits, margin_below, margin_above

# Sweep soft configurations around real CAD diameters
# LP DO: 20 to 24 mm
# HP DO: 34 to 40 mm
# bearing stiffnesses: k_lp from 0.15e8 to 0.35e8 N/m, k_hp from 8e8 to 12e8, k_is from 3e8 to 5e8
# We want to pull Mode 3 (BW3 and FW3) BELOW 18.9k RPM!
# Or we want to pull FW3 below 18.9k RPM!
test_cases = []
for lp_do in [0.020, 0.021, 0.022]:
    lp_di = lp_do - 0.008 # 4mm wall thickness
    for hp_do in [0.034, 0.035, 0.036]:
        hp_di = lp_do + 0.002 # 1mm clearance
        for k_lp in [0.15e8, 0.20e8, 0.25e8]:
            for k_hp in [8.0e8, 10.0e8]:
                for k_is in [3.0e8, 4.0e8]:
                    test_cases.append((lp_do, lp_di, hp_do, hp_di, k_lp, k_hp, k_is))

print(f"Checking {len(test_cases)} soft configurations...", flush=True)
for idx, tc in enumerate(test_cases):
    safe, crits, mb, ma = run_eval(*tc)
    if safe:
        print(f"FOUND SAFE SOFT CONFIG {idx+1}: lp_do={tc[0]*1000:.1f} mm, hp_do={tc[2]*1000:.1f} mm, k_lp={tc[4]/1e6:.1f} MN/m, k_hp={tc[5]/1e6:.1f} MN/m, k_is={tc[6]/1e6:.1f} MN/m")
        print(f"  crits={crits}, margin_below={mb:.1f}%, margin_above={ma:.1f}%", flush=True)
print("Done soft check.", flush=True)
