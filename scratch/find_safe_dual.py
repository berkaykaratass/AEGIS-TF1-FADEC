...
def run_simulation(lp_do, lp_di, hp_do, hp_di, k_lp, k_hp, k_is):
...
def main():
    # Sweep larger LP and HP diameters
    # LP do: 35 to 45 mm
    # HP do: lp_do + 2mm (clearance) + HP wall thickness
    # k_lp = 1.0e8 to 3.0e8
    # k_hp = 12.0e8 to 18.0e8
    # k_is = 4.5e8 to 8.0e8
    found = []
    print("Searching wider parameter space...")
    for lp_do in [0.036, 0.038, 0.040, 0.042, 0.044, 0.046]:
        for lp_thick in [0.005, 0.006]:
            lp_di = lp_do - 2 * lp_thick
            hp_di = lp_do + 0.002
            for hp_thick in [0.006, 0.007, 0.008]:
                hp_do = hp_di + 2 * hp_thick
                for k_lp in [1.5e8, 2.5e8]:
                    for k_hp in [12.0e8, 16.0e8]:
                        for k_is in [5.0e8, 8.0e8]:
                            safe, crits, mb, ma = run_simulation(lp_do, lp_di, hp_do, hp_di, k_lp, k_hp, k_is)
                            if safe:
                                found.append((lp_do, lp_di, hp_do, hp_di, k_lp, k_hp, k_is, crits, mb, ma))
                                print(f"Found safe config: LP DO={lp_do*1000:.1f}mm, HP DO={hp_do*1000:.1f}mm")
    
    print(f"\nTotal safe configs found: {len(found)}")
    found.sort(key=lambda x: x[0] + x[2]) # sort by diameter sum
    for idx, f in enumerate(found[:5]):
        print(f"\nSafe Config {idx+1}:")
        print(f"  LP Shaft: D_outer={f[0]*1000:.1f} mm, D_inner={f[1]*1000:.1f} mm (Wall: {(f[0]-f[1])*500:.1f} mm)")
        print(f"  HP Shaft: D_outer={f[2]*1000:.1f} mm, D_inner={f[3]*1000:.1f} mm (Wall: {(f[2]-f[3])*500:.1f} mm)")
        print(f"  Stiffness: k_lp={f[4]/1e6:.1f} MN/m, k_hp={f[5]/1e6:.1f} MN/m, k_is={f[6]/1e6:.1f} MN/m")
        print(f"  Critical Speeds [RPM]: {[int(x) for x in f[7]]}")
        print(f"  Margin Below Idle: {f[8]:.1f} %")
        print(f"  Margin Above Max: {f[9]:.1f} %")
